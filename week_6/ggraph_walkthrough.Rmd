...
The [tidygraph](https://tidygraph.data-imaginist.com/index.html) package extends that paradigm to networks, by representing networks as two tables -- a table of nodes and node attributes and a table of edges and edge attributes.
...
There are lots of layout options included with ggraph. You can let ggraph pick for you or you can look through some [here](https://igraph.org/c/doc/igraph-Layout.html) and [here](https://ggraph.data-imaginist.com/articles/Layouts.html).
...
And unfortunately, it is even worse when you resize nodes. I cannot figure out a good way to automatically adjust where the arrows start and stop when node sizes change. I think that there is a strategy that is supposed to work but it isn't working now (I [reported a bug](https://github.com/thomasp85/ggraph/issues/282)).

For now, I think it's just not a good idea to resize nodes in graphs where you want to display arrows for directed edges.
