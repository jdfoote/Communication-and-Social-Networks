...
The code to do this is a bit more complicated - basically, we take our node spreadsheet and temporarily convert it back to a "normal" R spreadsheet (a.k.a., `tibble`), and then visualize it. That's what the `as_tibble()` line below does.

Then, instead of using `ggraph`, we use `ggplot`, which is one of R's general plotting packages.
...
ffe_elite %>% as_tbl_graph() %>%
  ...
  ggplot() + 
  geom_boxplot(aes(fill=religion, y=degree)) + # `fill` changes the filled in color. Try changing this to `color` instead
  theme_minimal()
