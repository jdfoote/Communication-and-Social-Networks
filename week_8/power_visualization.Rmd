...
Let's start with this network. There are a few new things going on here, so I'll walk through each line:

* `set.seed(23)`: We're going to be creating a random network. By setting a "seed", we guarantee we all get the same network.
* `G = as_tbl_graph(random.graph.game(8, .25, directed = T))`: Here's our random graph - 8 nodes, a .25 probability of each edge, and it's directed
...
* `geom_node_text(aes(label = 1:8))`: This adds the labels 1 through 8 to the nodes. It just makes it easier for us to talk about individual nodes.
...
G %>%
  ...
  geom_node_text(aes(label = 1:8)) + # Adds labels to nodes
...
G %>%
  ...
  geom_node_text(aes(label = 1:8, size=degree *1.2)) +
  scale_edge_alpha('Edge direction', guide = 'edge_direction') + # Adds the "Edge direction" legend
...
G %>%
  ...
  geom_node_text(aes(label = 1:8, size=centrality * 1.2)) +
  scale_edge_alpha('Edge direction', guide = 'edge_direction') + # Adds the "Edge direction" legend
