...
4. *If you are using a Mac or Linux, you may have to add '~/' to the beginning of the file path.

```{r}
library(readxl)
#edgelist <- read_excel("~/Teaching/communication_and_networks/week_4/edgaelist.xlsx")
#View(edgelist)
```
...
Unfortunately, it's a little bit tricky to do that for this package. Try to run `install.packages("remotes")` in the Console, and when that is done run `remotes::install_github("schochastics/networkdata")`, also in the console.


If it works, then you should be able to load the package.
