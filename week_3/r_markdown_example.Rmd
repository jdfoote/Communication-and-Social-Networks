...
Try executing this chunk by clicking the *Run* button ![](./images/play_button.png) or by placing your cursor inside the code chunk it and pressing `Ctrl+Shift+Enter`. 
...
Code that will be run is marked with three backticks (\`) followed by `{r}` and ends with three backticks. The simplest way to create a code block is to do `Ctrl+Alt+I` or to click the green `Insert` button ![](./images/play_button.png) at the top of the Markdown window.
